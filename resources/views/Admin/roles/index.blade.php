                ...
                <tbody>
                    @foreach ($roles as $key => $role)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-sm btn-clean btn-icon" href="{{ route('roles.show', $role->id) }}"
                                    title="Show"><i class="la la-eye"></i></a>

                                @can('role-edit')
                                    <a class="btn btn-sm btn-clean btn-icon" href="{{ route('roles.edit', $role->id) }}"
                                        title="Edit"><i class="la la-edit"></i></a>
                                @endcan
                                @if ($role->name !== 'Admin')
                                    @can('role-delete')
                                        <a class="btn btn-sm btn-clean btn-icon" data-bs-toggle="modal"
                                            data-bs-target="#kt_modal_new_card-{{ $role->id }}" title="Delete"><button
                        ...
                        <div class="modal fade" id="kt_modal_new_card-{{ $role->id }}" tabindex="-1"
                            aria-hidden="true">
